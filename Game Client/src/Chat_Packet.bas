...
Public Sub SendMapMessage(ByVal Text As String)
    ...
    Buffer.WriteLong ChatChannel.chMap
...
Public Sub SendBroadcastMessage(ByVal Text As String)
    ...
    Buffer.WriteLong EnginePacket.PBroadcastMessage
    Buffer.WriteLong ChatChannel.chGlobal
    Buffer.WriteString Text
...
Public Sub HandleBroadcastMessage(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    ...
    Select Case Channel
    Case ChatChannel.chMap
        Header = "[Mapa] "

    Case ChatChannel.chGlobal
        Header = "[Global] "

    Case ChatChannel.chGame
        Header = "[Jogo] "

    Case ChatChannel.chParty
        Header = "[Party] "
        
    Case ChatChannel.chGuild
        Header = "[Clã] "
                
    Case ChatChannel.chPrivate
        Header = "[Privado] "
    End Select
...
Public Sub SendPlayerMessage(ByVal Text As String, ByVal MsgTo As String)
    ...
    Buffer.WriteLong EnginePacket.PBroadcastMessage
    Buffer.WriteLong ChatChannel.chPrivate
    Buffer.WriteString Text
