...
Private Sub CheckBoxChat_Game()
    Options.ChannelState(ChatChannel.chGame) = Windows(WindowIndex).Controls(GetControlIndex("winChat", "chkGame")).Value
    UpdateChat
End Sub

Private Sub CheckBoxChat_Map()
    Options.ChannelState(ChatChannel.chMap) = Windows(WindowIndex).Controls(GetControlIndex("winChat", "chkMap")).Value
    UpdateChat
End Sub

Private Sub CheckBoxChat_Global()
    Options.ChannelState(ChatChannel.chGlobal) = Windows(WindowIndex).Controls(GetControlIndex("winChat", "chkGlobal")).Value
    UpdateChat
End Sub

Private Sub CheckBoxChat_Party()
    Options.ChannelState(ChatChannel.chParty) = Windows(WindowIndex).Controls(GetControlIndex("winChat", "chkParty")).Value
    UpdateChat
End Sub

Private Sub CheckBoxChat_Guild()
    Options.ChannelState(ChatChannel.chGuild) = Windows(WindowIndex).Controls(GetControlIndex("winChat", "chkGuild")).Value
    UpdateChat
End Sub

Private Sub CheckBoxChat_Private()
    Options.ChannelState(ChatChannel.chPrivate) = Windows(WindowIndex).Controls(GetControlIndex("winChat", "chkPrivate")).Value
    UpdateChat
End Sub
...
Public Sub AddPlayerMessage(ByVal Text As String, ByRef MsgTo As String)
    ...
    AddText ColourChar & GetColStr(Color) & "[Privado] Para " & MsgTo & ": " & ColourChar & GetColStr(Grey) & Text, Grey, , ChatChannel.chPrivate
End Sub
