...
Public Sub DragBox_Check()
    ...
    Else
        ...
        Case PartTypeOrigins.origin_Inventory
            Dialogue "DESTRUIR ITEM", "Deseja realmente destruir o item?", "", TypeDESTROYITEM, StyleYESNO, DragBox.Slot
...
Sub btnOptions_Confirm()
    ...
    If Options.Music <> Value Then
        ...
        Else
            ...
            ' play music
            If InGame Then MusicFile = Trim$(CurrentMap.MapData.Music) Else MusicFile = Trim$(MenuMusic)
    ...
    If Options.NoAuto <> Value Then
        ...
        If Value = 0 Then
            If InGame Then
                AddText "Autotiles ativados.", BrightGreen
            End If
        Else
            If InGame Then
                AddText "Autotiles desativados.", BrightGreen
            End If
        End If
    End If
...
Sub btnInvite_Party()
    ...
    Dialogue "Party Invitation", diaDataString & " convidou voce para um grupo.", "Voce gostaria de aceitar?", TypePARTY, StyleYESNO
End Sub
...
' Se nao ha alvo, fecha a janela e sai do metodo.
    If MyTarget <= 0 Or MyTargetType = TargetTypeLoot Then
    ...
    If MyTargetType = TargetTypeNpc Then
        ...
        If NpcNum > 0 Then
            ...
            Windows(WindowIndex).Controls(ControlHPIndex).Text = MapNpc(MyTarget).Vital(HP) & "/" & MapNpc(MyTarget).MaxVital(HP)
            Windows(WindowIndex).Controls(ControlSPIndex).Text = MapNpc(MyTarget).Vital(MP) & "/" & MapNpc(MyTarget).MaxVital(MP)

            If MapNpc(MyTarget).Vital(HP) > 0 Then
                Percentage = CSng(MapNpc(MyTarget).Vital(HP) / MapNpc(MyTarget).MaxVital(HP))
            ...
            If MapNpc(MyTarget).Vital(MP) > 0 Then
                Percentage = CSng(MapNpc(MyTarget).Vital(MP) / MapNpc(MyTarget).MaxVital(MP))
                Width = 209 * Percentage
    ...
    ElseIf MyTargetType = TargetTypePlayer Then
        ...
        Windows(WindowIndex).Controls(ControlNameIndex).Text = "Lv. " & Player(MyTarget).Level & " " & Trim$(Player(MyTarget).Name)
        Windows(WindowIndex).Controls(ControlHPIndex).Text = Player(MyTarget).Vital(HP) & "/" & Player(MyTarget).MaxVital(HP)
        Windows(WindowIndex).Controls(ControlSPIndex).Text = Player(MyTarget).Vital(MP) & "/" & Player(MyTarget).MaxVital(MP)

        If GetPlayerVital(MyTarget, Vitals.HP) > 0 Then
            Percentage = CSng(GetPlayerVital(MyTarget, Vitals.HP) / GetPlayerMaxVital(MyTarget, Vitals.HP))
            Width = 209 * Percentage
        ...
        If GetPlayerVital(MyTarget, Vitals.MP) > 0 Then
            Percentage = CSng(GetPlayerVital(MyTarget, Vitals.MP) / GetPlayerMaxVital(MyTarget, Vitals.MP))
            Width = 209 * Percentage
